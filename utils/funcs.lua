function Timer(delay, callback)
    ...
    if GameRules.___TimerEntity__ == nil then GameRules.___TimerEntity__ = SpawnEntityFromTableSynchronous("info_target",{}) end
    GameRules.___TimerEntity__:SetContextThink(timerName,function()
        if GameRules.__vTimerNameTable__[timerName] then
            -- return callback()
            local success, ret = pcall(callback)
            if success then
                return ret
            else
                print(debug.traceback(ret))
                return 1 -- 如果运行失败，1秒之后重试，避免有运行失败的情况导致不运行了
            end
        else
            return nil
        end
...
function ShowError(msg, player_id, continue)
    Notifications:Bottom(PlayerResource:GetPlayer(player_id), { text = msg, duration = 1, style = { color = "red", ["font-size"] = "40px", border = "0px" } , continue = continue})
end
